           ...
           # self.logger.debug("read journal body ")
            for i in range(0, self.page_counter):
                ...
                if not isinstance(result, int):
                    x = {
                        'object_id': 1,
                        'page_type': 'transcations',
                        'page_id': i+1,
                        'offset': page_offset,
                    }
                    alldata.append(x)
                    self.rgen.generateReport(i, result, alldata,schema=["No schema found"])
        #self.logger.debug("end body pardsing")
        self.rgen.jsonconvert(self.outname, super(JournalParser, self)._path_leaf(self.filename),self.page_size)
