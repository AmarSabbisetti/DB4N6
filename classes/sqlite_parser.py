...
class SQLiteParser(Parser):
    ...
    def _parse_header(self):
        ...
        self.file_format_read_version = self.sqlite_header[2]
    ...
    """parsing the schema written on page one in the SQLite master table"""
    def _parse_schema(self):
        with open(self.filename, "rb") as f:
            ...
            #print(self.schema_related_pages)
            ...
            #print(d)
            #print(alllist)
            #for obj_id,sch_id in d["schema"].items():
            for i in range(len(alllist)):
                alllist[i]['schema']=d['schema'][alllist[i]['object_id']]
        ...
        with open(self.filename, "rb") as binary_file:
            ...
            for counter in range(1, int(max)):
                ...
                if not isinstance(result, int):
                    ...
                    #print(result)
                    self.rgen.generateReport(self.current_page, result, alllist, schema=self.current_schema)
                ...
                if freeblocks:
                    if len(freeblocks) > 0:
                        ...
                        self.rgen.generate_freeblock_report(os.path.abspath(self.outname + "/" + super(SQLiteParser, self)._path_leaf(self.filename) + "/freeblocks/" + "/"),str(counter) + "-page-freeblocks", freeblocks)
                ...
                if result != 2 and result != 10:
                    ...
                    if result:
                        ...
                        self.rgen.generateReport(counter, result, alllist, schema=self.current_schema)
                    counter += 1

        #self._parse_freelists()
        self.rgen.jsonconvert(self.outname, super(SQLiteParser, self)._path_leaf(self.filename),self.page_size)
    ...
    def _extract_cell(self, estimated_varint_length, freeblock, schema):
        ...
        if schema[0] != 'INT':
            raise TypeError("first column in the schema != an integer")
        ...
        for k in self.parsed_schemas:
            ...
            if k > 0:
                with open(self.filename, "rb") as f:
                    ...
                    if page_header[0] == 5:
                        #pageid.append(page_header[5])
                        t_offsets.append(page_header[3])
                        ...
                        for x in range(num_of_cells_in_frame):
                            ...
                            #print(cell_offset)
                            t_offsets.append(cell_offset)
                            #.append(page_header[5])
                            cell = unpack('>I', current_page[cell_offset:cell_offset + 4])
                           # print(cell,cell_offset)
                            result[k].append(cell[0])
                    else:

                        result[k].append(k)
                        t_offsets.append(page_header[3])
                        #t_offsets.append(page_header[5])
